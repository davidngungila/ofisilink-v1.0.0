...
@section('title', 'Overtime Management - OfisiLink')
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div class="mb-3 mb-md-0">
                            <h3 class="mb-2 text-white fw-bold">
                                <i class="bx bx-time-five me-2"></i>Overtime Management
                            </h3>
                            <p class="mb-0 text-white-50 fs-6">
                                Manage monthly overtime hours and calculations for employees
                            </p>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <h3 class="mb-0 fw-bold text-primary">{{ number_format($totalHours ?? 0, 2) }}</h3>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <small class="text-muted">With Overtime</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm h-100 border-warning" style="border-left: 4px solid #f59e0b !important;">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h6 class="text-muted mb-1 small">Avg Hours</h6>
                            <h3 class="mb-0 fw-bold text-warning">{{ number_format($avgHours ?? 0, 2) }}</h3>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card border-0 shadow-lg">
                <div class="card-body">
                    <form method="GET" action="{{ route('payroll.overtime.index') }}" class="d-flex gap-3 align-items-end">
                        <div class="flex-grow-1">
                        ...
                        <button type="button" class="btn btn-success" onclick="showAddOvertimeModal()">
                            <i class="bx bx-plus me-1"></i>Add Overtime
                        </button>
                        <button type="button" class="btn btn-info" onclick="showBulkOvertimeModal()">
                            <i class="bx bx-layer me-1"></i>Bulk Create
                        </button>
                        <button type="button" class="btn btn-warning" onclick="downloadOvertimeTemplate()">
            ...
            <div class="card border-0 shadow-lg">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0 text-white fw-bold">
                        <i class="bx bx-table me-2"></i>Overtime Records - {{ \Carbon\Carbon::parse($selectedMonth . '-01')->format('F Y') }}
                    </h5>
                </div>
                            ...
                            <tbody>
                                @forelse($employees as $employee)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            @if($overtime)
                                                <span class="fw-bold">{{ number_format($overtime->hours, 2) }}</span>
                                            @else
                                        ...
                                        <td class="text-end">
                                            @if($overtime)
                                                <span class="text-muted">TZS {{ number_format($overtime->hourly_rate, 2) }}</span>
                                            @else
                                        ...
                                        <td class="text-end">
                                            @if($overtime)
                                                <span class="fw-bold text-success">TZS {{ number_format($overtime->amount, 0) }}</span>
                                            @else
                                        ...
                                        <td>
                                            @if($overtime)
                                                <small class="text-muted">{{ Str::limit($overtime->description ?? 'N/A', 30) }}</small>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($overtime)
                                                <button class="btn btn-sm btn-primary" onclick="editOvertime({{ $overtime->id }}, {{ $employee->id }}, '{{ $employee->name }}', {{ $overtime->hours }}, '{{ $overtime->description ?? '' }}')">
                                                    <i class="bx bx-edit"></i>
                                                </button>
                                                <button class="btn btn-sm btn-danger" onclick="deleteOvertime({{ $overtime->id }})">
                                                    <i class="bx bx-trash"></i>
                                                </button>
                                            @else
                                                <button class="btn btn-sm btn-success" onclick="addOvertime({{ $employee->id }}, '{{ $employee->name }}')">
...
<div class="modal fade" id="overtimeModal" tabindex="-1" style="z-index: 10090 !important;">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="overtimeModalTitle">Add Overtime</h5>
            ...
            <form id="overtimeForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="hours" id="hours" class="form-control" step="0.01" min="0" max="744" required>
                        <small class="text-muted">Maximum 744 hours per month</small>
                    </div>
                    <div class="mb-3">
...
<!-- Bulk Create Overtime Modal -->
<div class="modal fade" id="bulkOvertimeModal" tabindex="-1" style="z-index: 10090 !important;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title">Bulk Create Overtime</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form id="bulkOvertimeForm" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="month" value="{{ $selectedMonth }}">
                    <div class="alert alert-info">
                        <i class="bx bx-info-circle me-2"></i>
                        <strong>Bulk Create Overtime</strong><br>
                        Upload a CSV or Excel file to create multiple overtime records at once. CSV format is recommended for best compatibility.
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Upload File <span class="text-danger">*</span></label>
                        <input type="file" name="file" id="bulk_overtime_file" class="form-control" accept=".csv,.xlsx,.xls" required>
                        ...
                        <div class="card bg-light">
                            <div class="card-body">
                                <small class="text-muted">
                                    ...
                                    - hours: Overtime hours (numeric, max 744, required)<br>
                                    - description (optional): Description of overtime work
                                </small>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
<script>
let overtimeModal = new bootstrap.Modal(document.getElementById('overtimeModal'));
let bulkOvertimeModal = new bootstrap.Modal(document.getElementById('bulkOvertimeModal'));

function showBulkOvertimeModal() {
    document.getElementById('bulkOvertimeForm').reset();
    bulkOvertimeModal.show();
}

function downloadOvertimeTemplate() {
    window.location.href = '/payroll/overtime/template';
}

function showAddOvertimeModal() {
    document.getElementById('overtimeModalTitle').textContent = 'Add Overtime';
    document.getElementById('overtimeForm').reset();
    document.getElementById('overtime_id').value = '';
    document.getElementById('employee_id').value = '';
    document.getElementById('employee_name').value = '';
    overtimeModal.show();
}

function addOvertime(employeeId, employeeName) {
    document.getElementById('overtimeModalTitle').textContent = 'Add Overtime';
    document.getElementById('overtimeForm').reset();
    document.getElementById('overtime_id').value = '';
...
function editOvertime(id, employeeId, employeeName, hours, description) {
    document.getElementById('overtimeModalTitle').textContent = 'Edit Overtime';
...
function deleteOvertime(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: 'This will deactivate the overtime record.',
        icon: 'warning',
...
$('#overtimeForm').submit(function(e) {
    ...
    const overtimeId = $('#overtime_id').val();
    const url = overtimeId ? `/payroll/overtime/${overtimeId}` : '/payroll/overtime';
    const method = overtimeId ? 'PUT' : 'POST';
...
$('#bulkOvertimeForm').submit(function(e) {
    e.preventDefault();
