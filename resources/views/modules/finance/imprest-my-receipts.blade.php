...
@section('title', 'My Receipts for Imprest Assignment')

@section('breadcrumb')
<div class="db-breadcrumb">
    <h4 class="breadcrumb-title">My Receipts for Imprest Assignment</h4>
...
<div class="container-fluid px-4 py-3">
    ...
    <div class="mb-3 d-flex gap-2 flex-wrap">
        ...
        <a href="{{ route('imprest.my-assignments') }}" class="btn btn-outline-secondary btn-sm">
            <i class="bx bx-clipboard me-1"></i>My Assignments
        </a>
        @if($assignment->imprestRequest->status === 'paid')
        <a href="{{ route('imprest.submit-receipt.page', $assignment->id) }}" class="btn btn-primary btn-sm">
            <i class="bx bx-plus-circle me-1"></i>Submit New Receipt
        </a>
        @endif
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4 bg-primary">
        <div class="card-body text-white">
            <h2 class="fw-bold mb-2 text-white">
                <i class="bx bx-receipt me-2"></i>My Receipts for Imprest Assignment
            </h2>
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p class="mb-0">
                        ...
                        @if($assignment->receipt_submitted)
                            ...
                            @if($assignment->receipt_submitted_at)
                                <small class="text-muted">({{ $assignment->receipt_submitted_at->format('d M Y, H:i') }})</small>
                            @endif
                        @else
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            ...
            @if($assignment->imprestRequest->status === 'paid')
            <a href="{{ route('imprest.submit-receipt.page', $assignment->id) }}" class="btn btn-light btn-sm">
                <i class="bx bx-plus-circle me-1"></i>Submit New Receipt
            </a>
            @endif
        </div>
        <div class="card-body">
                        ...
                        <div class="card receipt-card {{ $receipt->is_verified ? 'verified' : 'pending' }}">
                            <div class="card-body">
                                ...
                                <div class="d-flex gap-2 mt-3">
                                    ...
                                    @endif
                                    @if($assignment->imprestRequest->status === 'paid')
                                    <a href="{{ route('imprest.submit-receipt.page', $assignment->id) }}" class="btn btn-sm btn-outline-success">
                                        <i class="bx bx-plus-circle"></i> Add Another Receipt
                                    </a>
                                    @endif
                                </div>
                            </div>
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">You haven't submitted any receipts for this assignment yet.</p>
                    @if($assignment->imprestRequest->status === 'paid')
                    <a href="{{ route('imprest.submit-receipt.page', $assignment->id) }}" class="btn btn-primary mt-2">
                        <i class="bx bx-plus-circle me-1"></i>Submit Your First Receipt
                    </a>
                    @else
                    <div class="alert alert-info mt-3 mb-0">
                        <i class="bx bx-info-circle me-2"></i>This imprest request has not been paid yet. You can submit receipts after payment is processed.
                    </div>
                    @endif
                </div>
            @endif
