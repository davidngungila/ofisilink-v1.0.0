...
<div class="container-fluid px-4 py-3">
    ...
    <div class="card border-0 shadow-sm mb-4" style="background:#940000;">
		...
		<div>
                    ...
                    <p class="mb-0 opacity-90">Unified cash-out ledger across Petty Cash, Imprest, and Payroll</p>
                </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" class="form-control" id="filterQ" placeholder="Search by description, party, reference, or account...">
                ...
                <table class="table table-hover mb-0" id="ledgerTable">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="sortable" data-sort="ref">
...
<script>
(function(){
	const endpoint = '{{ route('modules.finance.ledger.data') }}';
    const pdfEndpoint = '{{ route('modules.finance.ledger.pdf') }}';
    ...
    function getAccountBadge(account){
        ...
        return badges[account] || 'bg-secondary';
    }
    ...
    function renderTable(entries){
        ...
        tbody.innerHTML = entries.map((e, idx) => `
            <tr class="fade-in" style="animation-delay: ${idx * 0.02}s">
                <td>
                    <span class="badge bg-light text-dark">${e.date ?? ''}</span>
                </td>
                <td>
                    <div class="fw-medium">${escapeHtml(e.description ?? '')}</div>
                    ${e.party ? `<small class="text-muted">${escapeHtml(e.party)}</small>` : ''}
                </td>
                <td>
                    <span class="badge ${getAccountBadge(e.account)}">${escapeHtml(e.account ?? '')}</span>
                </td>
                <td><code class="text-primary">${escapeHtml(String(e.ref ?? ''))}</code></td>
                <td class="text-end">
                ...
                <td>
                    <i class="bx ${getSourceIcon(e.source)} text-muted" title="${escapeHtml(e.source ?? '')}"></i>
                </td>
            </tr>
        `).join('');
    }
    ...
    document.getElementById('btn-export').addEventListener('click', (e) => {
        ...
        // Remove pagination params for PDF export
        p.delete('page');
        p.delete('per_page');
